...
> **分析日期**: 2026-03-19 (Updated: 2026-05-05 for v0.4.5 bridge reliability hardening)
> **源文件**: `cmd/app/fm_http.go`
> **框架**: Echo v4 (Go)
> **默认端口**: 5532 (可通过 `FM_PORT` 环境变量配置)
> **向量引擎**: Faiss (legacy) / Zvec (recommended, v0.4.5+)
> **进程退出**: SIGINT/SIGTERM 走 `e.Shutdown(ctx, 30s)` + `index.FreeAllActiveWrappers()` 兜底，不再用 `os.Exit(0)` 硬切；详见 [Zvec 集成指南 §11](../guides/zvec_integration_guide_cn.md#11-%E5%8F%AF%E9%9D%A0%E6%80%A7%E4%B8%8E%E6%A1%A5%E8%BF%9B%E7%A8%8B%E7%94%9F%E5%91%BD%E5%91%A8%E6%9C%9F)
