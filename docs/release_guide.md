...
假设我们要发布的新版本号为 `v0.1.1`（注：NPM和PyPI不需要 `v` 前缀，即 `0.1.1`）。
...
**✅ 检查状态：**
前往 GitHub 仓库的 [Actions](https://github.com/ZetaZeroHub/FlashMemory/actions) 页面，等待构建任务（`Release`）变绿并成功完成。一旦成功，去项目的 Releases 页面将会看到生成了 `v0.1.1` 和对应的 5 个压缩文件。
...
git add .
git commit -m "update formula to v0.1.1"
...
# 将 "version": "0.1.1" 改为 "version": "0.1.1"
...
# 修改包声明的版本号（共两个文件）
# 1. 将 pyproject.toml 里的 version = "0.1.1" 改为 "0.1.1"
# 2. 将 flashmemory/__init__.py 里的 __version__ = "0.1.1" 改为 "0.1.1"
